...
for snrIdx = 1:numel(SNRIn)
    ...
    Peak = zeros(32,1);
    RMS = zeros(32,1);
    pdschSymbolsNum = gnb.NDLRB*12*12-gnb.NDLRB*12*(4+4);
...
%             [pdschGrid,csiisindices] = CSIRSmapping_new_15812(pdschGrid,CSIRS(ind1,:),gnb,port);
            [pdschGrid,csiisindices] = CSIRSmapping_new_2345891011(pdschGrid,CSIRS(ind1,:),gnb,port);
             %DMRS mapping
%             if port>=1 && port<=12              
...
%         csiposition=[gnb.NDLRB*12*0+1:gnb.NDLRB*12*1 gnb.NDLRB*12*4+1:gnb.NDLRB*12*5 gnb.NDLRB*12*7+1:gnb.NDLRB*12*8 gnb.NDLRB*12*11+1:gnb.NDLRB*12*12];
        csiposition=[gnb.NDLRB*12*1+1:gnb.NDLRB*12*5 gnb.NDLRB*12*7+1:gnb.NDLRB*12*11];
...
%             pdschGrid = CSIRSmapping_new_15812(pdschGrid,CSIRS(ind1,:),gnb,port);
            [pdschGrid,~] = CSIRSmapping_new_2345891011(pdschGrid,CSIRS(ind1,:),gnb,port); 
...
%             hD = ChannelEstimate_new_15812_alleles(grid,txgrid,CSIRSIndices,DMRSIndices);
             hD = ChannelEstimate_new_2345891011_alleles(grid,txgrid,CSIRSIndices);%1320x12x32x32
...
%
            ...
            for sc=1:size(grid,1)
             ...
             G_all_li(:,:,:,BW)=ExtGallResponse(G_all,pdschIndices);%32x32x5280x2
             fprintf('RV 计算已完成 .\n');
%              if BW==5
%              continue;
%              end
...
%             pdschTx = ExtractResources(PDSCHIndices,txgrid);
            if nn==1
%             chEst = ExtChResponse(hD, PDSCHIndices)/9; %ori
            chEst = ExtChResponse(hD, PDSCHIndices); %new 去掉9.
...
%                 pdsch_origin=pdsch; 
            ...
            err_allports=0;
              for port=1:32
                  [rxCws,symbols] = ltePDSCHDecode_modify(gnb_origin, pdsch_origin, pdschRxEq(:,port));%8360x1
                  codedTrBlock_Eq=rxCws{1,1};
                  codedTrBlock_Eq(codedTrBlock_Eq<0)=0;
                  codedTrBlock_Eq(codedTrBlock_Eq>0)=1;
%                   codedTrBlock_Eq(codedTrBlock_Eq == 0) = -1;
                  decState = [];% isequal(codedTrBlockall(:,port,BW),codedTrBlock_Eq)
%                   [rxdata,~,decState] = h5gDLSCHDecode_modify(gnb,pdsch,trBlk,codedTrBlock_Eq,decState);%仅仅是一个子带的接收判决后的数据.h5gDLSCHDecode
%                   [rxdata,~,decState] = h5gDLSCHDecode(gnb_origin,pdsch_origin,trBlk,codedTrBlockall(:,port,BW),decState);
                  err_port=length(find(codedTrBlockall(:,port,BW)~=codedTrBlock_Eq));
...
%                   rxdataAll(:,port,BW)=rxdata;
                  ...
                  err_allports=err_allports+err_port;%一个子带上的所有用户的错误估计比特.
...
%                   axis([-1,1,-1,1].*2);
              ...
              err_allbws=err_allbws+err_allports;
%             figure
